...
void Gyro::setup_gyro() {
 ...
 writeReg(GYRO_ADR, 0x23, 0xA0);  // Block update until read, FS 2000 dps 
...
Vector3 Gyro::read_gyro() {
  if ((micros() - last_gyro_timestamp) < GYRO_REFRESH_PERIOD) return last_gyro_vec; 
  ...
  while (Wire.available()<6) {
    if ((micros() - last_gyro_timestamp) > SENSOR_TIMEOUT_US){
      gyro_timeout = 1;
      return zero_3vector;
    }  
  }
