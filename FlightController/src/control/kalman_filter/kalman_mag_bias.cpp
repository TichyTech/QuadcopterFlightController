...
const Vector3 MAG_VAR_BASE = {5, 5, 5};
...
KalmanFilterMB::KalmanFilterMB(){
  q = {1,0,0,0};
  b = {0,0,0};
  mag_bias = {-0.493,0.333,0.385};
  ...
  a_w = {0,0,1};
  m_w = {0.5f*cos(MAG_INC), 0, -0.5f*sin(MAG_INC)};
 ...
 */
void KalmanFilterMB::init_quat(Matrix3 DCM){
 ...
 */
void KalmanFilterMB::track_gyro(Vector3 w, float dt){
 ...
 */
void KalmanFilterMB::track_acc(Vector3 a, float dt){
  float a_norm = norm(a);
 ...
 */
bool KalmanFilterMB::gyro_steady(){
 ...
 * Whether or not the drone is steady enough
 */
bool KalmanFilterMB::acc_steady(){
 ...
 */
Vector4 KalmanFilterMB::predict(Vector3 gyro_vec, float dt){
  ...
  Zeros<3,3> Zeros3_3;
  Zeros<3,7> Zeros3_7;
  Matrix<10,10> F = (F11 || (0.5f*dt)*F12 || ~Zeros3_4) && (Zeros3_4 || I_3 || Zeros3_3) && (Zeros3_7 || I_3);  // construct the F matrix
  ...
  Q.Submatrix<3,3>(7,7) = I_3 * 0.00001f;
  Q = Q + I_10*(1e-5f*dt);  // baseline process noise
 ...
 */
Vector4 KalmanFilterMB::fuse_mag(Vector3 m, float th){
  // expected measurement h(x) = [R(q)a_w]
  Matrix3 Rq = quat2R(q);
  Matrix<3,1> hx = Rq*m_w + th*th*mag_bias;
  ...
  Matrix3 dh_dmb = th*th * I_3;
  // H = dh(x)/dx using derivative of d(R(q)x)/dq
  Matrix<3,10> H = 2.0f*((q0*m_w + v_x*m_w) || (I_3*dot(v,m_w) + v*~m_w -q0*skew(m_w) -m_w*~v)) || Zeros3_3 || dh_dmb;
  ...
  Matrix<10, 3> KS = P*~H;  // K = PH^TS^-1, KS = PH^T
  // S\H computation
  Matrix<3,10> SinvH;
  for (int i=0; i<10; i++){  // solve S^-1 * H(:,i) for each column of H
  ...
  Matrix<10,1> innovation = KS * CholeskySolve(LLT, error);
  P = (I_10 - KS*SinvH)*P;
  ...
  Vector3 inn_mb = innovation.Submatrix<3,1>(7,0);  // magnetic bias innovation
  ...
  if (th > 0.1f) mag_bias = mag_bias + 0.05f*inn_mb;
 ...
 */
Vector4 KalmanFilterMB::fuse_acc(Vector3 a, float th){
  // expected measurement h(x) = [R(q)a_w]
  Matrix3 Rq = quat2R(q);
  ...
  Matrix3 v_x = skew(v);  // skew symmetric from v
  // H = dh(x)/dx using derivative of d(R(q)x)/dq
  Matrix<3,10> H = 2.0f*((q0*a_w + v_x*a_w) || (I_3*dot(v,a_w) + v*~a_w -q0*skew(a_w) -a_w*~v)) || Zeros3_3 || Zeros3_3;
  // Compute the covariance matrix of the measurements and prepare Cholesky decomp
  Matrix<3,3> S = H*P*~H;  
  ...
  Matrix<10,1> innovation = KS * Sinverr;  // inn = P H^T S^-1 (z - h(x))
  P = (I_10 - KS*SinvH)*P;

  // Save result back to our state vectors
  Vector4 inn_q = innovation.Submatrix<4,1>(0,0);  // quaternion innovation
  Vector3 inn_b = innovation.Submatrix<3,1>(4,0);  // bias innovation
  Vector3 inn_mb = innovation.Submatrix<3,1>(7,0);  // bias innovation

  inn_q = clamp_innovation(inn_q, q);  // restrict quat innovations to small, orthogonal
  q = normalize(q + inn_q);  // add innovation and renormalize
  if(acc_steady()) b = b + inn_b;  // if acc not steady, use smaller step
  else b = b + 0.05f*inn_b;
//   if (th > 0.1f) mag_bias = mag_bias + inn_mb;

  return q;
}

float KalmanFilterMB::get_P_max()
{
    return max_norm(P);
}

float KalmanFilterMB::get_P_min()
{
    return min_norm(P);
}
 ...
 */
float KalmanFilterMB::clamp_variance()
{
 ...
 */
Vector4 KalmanFilterMB::clamp_innovation(Vector4 inn_q, Vector4 q)
{
 ...
 */
Matrix3 KalmanFilterMB::quat2R(Vector4 q){
  float q0 = q(0);  // scalar part
  Vector3 v = q.Submatrix<3,1>(1,0);  // vector part
  Matrix3 v_x = skew(v);  // skew symmetric from v
